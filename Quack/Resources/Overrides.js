...
const TRANSFORMERS = [
    ...
    {
        match: m => m.path.includes('Eorzean-Nightlife-V3') && m.tags.includes('emote'),
        mutate: (macro, macros, mods) => {
            ...
            if (matches.length == 1) {
                ...
                const mod = mods.find(m => m.name.includes('Eorzean-Nightlife-V3'));
                const otherEmoteGroupNames = mod.settings.groupSettings.map(s => s.name).filter(name => ['Default', 'Miscellaneous', 'Signboard (Turn 9 On)', 'Breed Expressions (Turn 7 On)', 'Chain Leash VFX (Imperial Salute)', match[1]].indexOf(name) === -1);
